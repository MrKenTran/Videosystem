...
class UserStatus extends LitElement {
   ...
   */
  firstUpdated(changedProperties, e) {
  ...
  render() {
    return html`
      ...
      <div class="user${this.hasAvatar?" avatar":""}" @click="${this.openStatus}">
        ...
        ${this.showStatus?
          ...
          <div class="status">
            <div>
              ${this.uname} logget på.
              <button @click="${this.logout}">Logg av</button>
            </div>
  ...
  register(e){
    ...
    .then(res=>{
      if (res.status=='SUCCESS') {  // Successfully logged in
        ...
        store.dispatch(re({uid: res.uid, uname: res.uname, isStudent: this.student, isTeacher: this.teacher, isAdmin: this.admin, hasAvatar: this.hasAvatar}));
      }                             // Needs to alert the user as to the error!!!!
    })
  }
